...
import { agents, applications, type Agent } from "@/lib/admin-data"
import { cn } from "@/lib/utils"

const channelOptions: Agent["channel"][] = ["Retail Partner", "Direct Sales", "Third-Party"]
const statusOptions: Agent["status"][] = ["Active", "Pending", "Suspended"]
...
export function AgentsTable() {
  ...
  const selectedAgentApps = useMemo(
    () => (selectedAgent ? applications.filter((app) => app.agentName === selectedAgent.name) : []),
    [selectedAgent],
  )
          ...
          {selectedAgent && (
            <>
              ...
              <div className="flex flex-col gap-4 px-4">
                <span
                  className={cn(
                    "inline-flex w-fit items-center rounded-md px-2 py-1 text-[11px] font-semibold tracking-wide uppercase",
                    statusStyles[selectedAgent.status],
                  )}
                >
